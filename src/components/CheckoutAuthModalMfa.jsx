...
export default function CheckoutAuthModalMfa({ open, onClose, onReady }) {
        ...
        <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between sticky top-0 bg-slate-950/95 z-10">
          <div>
            <h3 className="text-white font-semibold">
              {mode === "signup" ? "Crear cuenta" : mode === "mfa" ? "Verificacion en 2 pasos" : "Iniciar sesion"}
            </h3>
        ...
        <div className="p-5 space-y-4">
          ...
          {mode === "mfa" && (
            <form onSubmit={handleVerifyMfa} className="space-y-4">
              <p className="text-[12px] text-slate-300">Abre tu app de autenticacion y escribe el codigo de 6 digitos.</p>
              <div>
                <label className="block text-[11px] uppercase tracking-[0.16em] text-slate-300/80 mb-1">Codigo</label>
                <input
                  type="text"
                  inputMode="numeric"
                  maxLength={6}
